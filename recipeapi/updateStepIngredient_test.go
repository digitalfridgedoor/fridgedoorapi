...
func TestUpdateStepIngredient(t *testing.T) {
	...
	// Act
	r, err := editable.AddStepIngredient(ctx, 0, ing.ID)
	assert.Nil(t, err)
	...
	r, err = editable.UpdateStepIngredient(ctx, 0, ing.ID.Hex(), updates)
	assert.Nil(t, err)
	assert.Equal(t, "recipe", r.Name)
	assert.Equal(t, 1, len(r.Method))
	assert.Equal(t, 1, len(r.Method[0].Ingredients))
	...
	r, err = editable.RemoveStepIngredient(ctx, 0, ing.ID.Hex())
	assert.Nil(t, err)
	assert.Equal(t, "recipe", r.Name)
	assert.Equal(t, 1, len(r.Method))
